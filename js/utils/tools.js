...
export function htmlJsonExtract() {
  try {
    const assetContext = document.getElementById("assetContext").innerHTML;
    const docJsonData = document.getElementById("docJsonTxt").innerHTML;

    if (assetContext.includes("{%") || docJsonData.includes("{%")) {
      return false;
    } return { assetContext, data: JSON.parse(docJsonData) };
